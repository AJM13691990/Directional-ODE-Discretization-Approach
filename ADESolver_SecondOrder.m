...
d0=input('Enter the dispersion coefficient d0 (e.g., 0.0005): ');
beta=input('Enter the nonlinearity parameter beta (e.g., 0): ');
...
v_x_str=input('Enter the function for v_x as a function of (x,y,u) (e.g., @(x,y,u) 1 or simply 1 [ideally between between 0 and 7]): ','s');
...
v_y_str=input('Enter the function for v_y as a function of (x,y,u) (e.g., @(x,y,u) 1 or simply 1 [ideally between between 0 and 7]): ','s');
...
for i=1:Nx
    for j=1:Ny
        Vx(i,j)=v_x(x(i),y(j),u_old(i,j))*(1+rand_level*randn());
        Vy(i,j)=v_y(x(i),y(j),u_old(i,j))*(1+rand_level*randn());
    end
end
...
for n=1:Nt
    ...
    X_adv=X-(Vx)*dt/2;Y_adv=Y-(Vy)*dt/2;
...
cbar=colorbar;
set(cbar,'Color','k'); % show colorbar ticks and labels automatically
...
% Set ticks so that the axis range is visible
